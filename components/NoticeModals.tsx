            ...
            <div className="flex-1 overflow-y-auto p-6">
              {isAuthRequired ? (
                <div className="py-20 flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-slate-400 mb-6 text-sm">작업을 계속하려면 비밀번호를 입력하세요.</p>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="flex gap-2">
                    <button type="submit" className="flex-1 bg-indigo-500 py-4 rounded-xl font-bold hover:bg-indigo-400 text-white">등록하기</button>
                ...
                <div className="animate-fade-in">
                  ...
                  <div className="text-slate-300 whitespace-pre-wrap leading-relaxed text-base min-h-[200px]">{selectedNotice.content}</div>
                  <div className="mt-8 flex justify-end gap-2">
                    <button onClick={() => runWithAuth(() => deleteNotice(selectedNotice.id))} className="text-xs text-red-400 hover:bg-red-500/10 px-4 py-2 rounded-lg">삭제</button>
                    <button onClick={() => setSelectedNotice(null)} className="bg-white/5 text-white px-6 py-2 rounded-xl text-sm font-bold hover:bg-white/10">목록으로</button>
                  </div>
                </div>
              ) : (
                <div className="rounded-2xl border border-white/5 overflow-hidden">
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {isLoading ? (
                        <tr><td colSpan={4} className="py-20 text-center">불러오는 중...</td></tr>
                      ) : notices.length === 0 ? (
                        ...
                        notices.map(notice => (
                          <tr key={notice.id} className="hover:bg-white/5 group transition-colors">
                            <td className="px-6 py-4">
                              <button onClick={() => setSelectedNotice(notice)} className="text-white font-medium hover:text-indigo-400 text-left transition-colors truncate max-w-[200px] sm:max-w-xs">{notice.title}</button>
                            </td>
                            <td className="px-6 py-4 text-slate-500 text-xs">{new Date(notice.timestamp).toLocaleDateString()}</td>
                            <td className="px-6 py-4"><span className="text-xs font-bold text-slate-400">정찬T</span></td>
                            <td className="px-6 py-4 text-right">
                              <button onClick={() => runWithAuth(() => deleteNotice(notice.id))} className="text-slate-600 hover:text-red-400 text-xs">삭제</button>
                            </td>
                          </tr>
                        ))
                      )}
